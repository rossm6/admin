...
function useInputField(unprefixedName) {
  ...
  return {
    ...
    props: {
      ...
      isChecked,
      type,
      choices: state.fields[name].choices,
    },
  };
}
...
function prepareValuesForSubmission(formState) {
  ...
  Object.entries(formState.values).forEach(([fieldName, fieldValue]) => {
    if (formState.fields[fieldName].pipe) {
      valuesForSubmission[fieldName] = formState.fields[fieldName].pipe(fieldValue);
    } else if (formState.fields[fieldName].choices) {
